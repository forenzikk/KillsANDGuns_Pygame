...
class Sprites:
    ...
    def blocked_walls(self):
        blocked_walls = Dict.empty(
            key_type=types.UniTuple(
                int32, 2), value_type=int32)
...
class SpriteObject:
    def __init__(self, parameters, pos):
        ...
        # сторона квадрата, в котором будет спрайт
        self.side = parameters['side']
        self.dead_animation_count = 0
        self.animation_count = 0
        self.font_point = pygame.font.Font('font/font.ttf', 144)
        self.npc_action_trigger = False  # триггер выполнения действия
        ...
        if self.viewing_angles:
            ...
            self.sprite_positions = {
                angle: pos for angle, pos in zip(
                    self.sprite_angles, self.object)}

    @property
    def is_on_fire(self):  # определение спрайта под огнем по центральному лучу и вблизи него
        if center_of_ray - self.side // 2 < self.current_ray < center_of_ray + \
                self.side // 2 and self.blocked:
    ...
    def object_locate(self, player):
        ...
        # разница координат игрока и спрайтов
        dx, dy = self.x - player.x, self.y - player.y
        self.distance_to_sprite = math.sqrt(
            dx ** 2 + dy ** 2)  # вычисление расстояния между ними

        self.theta = math.atan2(dy, dx)  # рассчет углов
        gamma = self.theta - player.angle  # еще углы
        if dx > 0 and 180 <= math.degrees(
                player.angle) <= 360 or dx < 0 and dy < 0:  # эмпирическое условие для угла
        ...
        if 0 <= fake_ray <= fake_rays_range and self.distance_to_sprite > 30:
            # проекционная высота спрйта (ограниченная)
            self.proj_height = min(
                int(proj_coeff / self.distance_to_sprite), 800)
            ...
            sprite_position = (
                ...
                400 - half_sprite_height + shift)  # позиция относительно луча
            # масштабирование по размеру проекции
